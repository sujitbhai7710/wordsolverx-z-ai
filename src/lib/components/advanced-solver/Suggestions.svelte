...
{:else}
	<div class="w-full max-w-md mx-auto mt-6 p-4 rounded-lg bg-white shadow-lg border border-slate-100">
		<div class="text-center mb-4">
			...
			<h2 class="text-2xl font-bold text-slate-800">{possibilities} possibilities</h2>
			...
			<table class="w-full text-sm text-left text-slate-500">
				...
				<tbody>
					{#each suggestions.slice(0, visibleCount) as s, index}
						<tr
							class="bg-white border-b hover:bg-slate-50 cursor-pointer transition-colors"
							onclick={() => onSelectWord && onSelectWord(s.word)}
						>
							<td class="px-6 py-4 font-medium">{index + 1}.</td>
							<td class="px-6 py-4 font-bold text-slate-900">{s.word.toUpperCase()}</td>
							<td class="px-6 py-4 text-right text-teal-600 font-medium">
		...
		{#if visibleCount < suggestions.length}
			<div class="mt-4 text-center">
				<button
					...
					class="px-6 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-full transition-colors text-sm"
				>
