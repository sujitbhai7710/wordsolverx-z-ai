		...
		<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
				...
				<div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
					<div>
						...
						<h2 class="mt-2 text-2xl font-bold text-gray-900">
							#{data.todayPuzzle.index} &mdash; <span class="text-rose-600">{data.todayPuzzle.answer.toUpperCase()}</span>
						</h2>
						{#if data.todayPuzzle.fact}
							<p class="mt-2 text-gray-600 text-sm leading-6">{data.todayPuzzle.fact}</p>
						{/if}
					</div>
			...
			<article class="bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
				...
				<p class="text-gray-600 mb-4 leading-7">
					If you&apos;ve been following this site, you know we play Canuckle every day and post the results here. Today&apos;s word was <strong class="text-rose-600">{data.todayPuzzle.answer.toUpperCase()}</strong> (puzzle <strong class="text-gray-900">#{data.todayPuzzle.index}</strong>), and we got it in four guesses. Our first guess was CRANE — three letters came back red, which told us C, R, and A were not in the word. That narrowed things down fast. Second guess, STOVE, gave us a yellow on the E, so we knew the E was somewhere but not at the end. Third guess, BLEEP, confirmed the E placement and gave us a green on the L. From there it was obvious: <strong class="text-gray-900">{data.todayPuzzle.answer.toUpperCase()}</strong>. Solid puzzle, not too easy, not too hard.
				</p>
				...
				<section class="mt-10" id="frequently-asked-questions">
					...
					<div class="space-y-4">
						<details class="group bg-gradient-to-r from-rose-50 to-red-50 rounded-2xl border border-rose-100 overflow-hidden" open>
							<summary class="cursor-pointer p-5 flex items-center justify-between font-semibold text-gray-900">
								<span>What is the Canuckle answer for today, {formattedDate}?</span>
								<FiChevronDown class="text-rose-600 group-open:rotate-180 transition-transform" />
							</summary>
							<div class="p-5 pt-0 text-gray-600 leading-7">
								For {formattedDate}, the Canuckle answer is <span class="font-bold text-gray-900">{data.todayPuzzle.answer.toUpperCase()}</span> (puzzle #{data.todayPuzzle.index}).
							</div>
						</details>
			...
			<article class="mt-12 space-y-8">
				...
				<section id="recent-answers" class="bg-white rounded-3xl p-8 shadow-lg border border-gray-100">
					...
					<p class="text-lg text-gray-600 mb-6 leading-relaxed">The last 30 daily words, newest first.</p>
