<script lang="ts">
	let { data } = $props();

	let copied = $state(false);
	let h1Title = $derived(`Nerdle Answer Today ( ${data.formattedDate} )`);

	function getTileStyle(char: string, index: number): string {
		const isOperator = ['+', '-', '*', '/', '='].includes(char);
		const isEquals = char === '=';
		const background = isEquals
			? 'linear-gradient(135deg, #10b981 0%, #059669 100%)'
			: isOperator
				? 'linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%)'
				: 'linear-gradient(135deg, #334155 0%, #1e293b 100%)';
		const shadow = isEquals
			? '0 4px 20px rgba(16, 185, 129, 0.3)'
			: isOperator
				? '0 4px 20px rgba(139, 92, 246, 0.3)'
				: '0 4px 20px rgba(0, 0, 0, 0.2)';

		return `animation-delay: ${index * 100}ms; background: ${background}; box-shadow: ${shadow};`;
	}

	async function copyEquation(): Promise<void> {
		if (!data.answerData?.answer) return;
		await navigator.clipboard.writeText(data.answerData.answer);
		copied = true;
		setTimeout(() => {
			copied = false;
		}, 1400);
	}
</script>

<svelte:head>
	<title>{data.meta.title}</title>
	<meta name="description" content={data.meta.description} />
	<meta name="keywords" content={data.meta.keywords} />
	<link rel="canonical" href={data.meta.canonical} />
	<meta property="og:title" content={data.meta.title} />
	<meta property="og:description" content={data.meta.description} />
	<meta property="og:type" content="article" />
	<meta property="og:url" content={data.meta.canonical} />
	<meta property="og:image" content="https://wordsolver.tech/wordsolverx.webp" />
	<meta property="og:site_name" content="WordSolverX" />
	<meta name="twitter:card" content="summary_large_image" />
	<meta name="twitter:title" content={data.meta.title} />
	<meta name="twitter:description" content={data.meta.description} />
	<meta name="twitter:image" content="https://wordsolver.tech/wordsolverx.webp" />
	{@html `<script type="application/ld+json">${data.schemas}</script>`}
</svelte:head>

<main class="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 text-slate-900">
	<div class="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
		<div class="mx-auto max-w-2xl">
			<div class="text-center mb-8">
				<p class="text-slate-600 text-sm mb-2">Today&apos;s Puzzle</p>
				<h1 class="text-3xl sm:text-4xl font-black mb-2 leading-tight">{h1Title}</h1>
				<div class="inline-flex items-center gap-2 bg-white border border-emerald-100 rounded-full px-4 py-2 shadow-sm">
					<span class="text-slate-600">Puzzle #</span>
					<span class="text-emerald-700 font-bold">{data.answerData.puzzleNumber}</span>
				</div>
			</div>

			<div class="rounded-2xl border border-slate-200 bg-white p-8 mb-8 shadow-sm">
				<div class="text-center mb-6">
					<h2 class="text-lg font-semibold text-slate-700">Today&apos;s Nerdle Equation</h2>
				</div>

				{#if data.answerData.answer}
					<div class="flex justify-center gap-2 md:gap-3 flex-wrap">
						{#each data.answerData.answer.split('') as char, index}
							<div
								class="w-12 h-14 md:w-16 md:h-18 rounded-lg flex items-center justify-center text-2xl md:text-3xl text-white font-bold transition-all duration-300 hover:scale-105"
								style={getTileStyle(char, index)}
							>
								{char}
							</div>
						{/each}
					</div>
				{:else}
					<div class="text-center py-8">
						<div class="animate-spin rounded-full h-12 w-12 border-4 border-emerald-500 border-t-transparent mx-auto mb-4"></div>
						<p class="text-slate-500">Loading answer...</p>
					</div>
				{/if}
			</div>

			{#if data.answerData.answer}
				<div class="rounded-xl border border-slate-200 bg-white p-6 mb-8 shadow-sm">
					<h3 class="text-lg font-semibold text-slate-900 mb-4">Equation Breakdown</h3>
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
						<div>
							<span class="text-slate-600">Full Equation:</span>
							<p class="text-slate-900 font-mono text-lg mt-1">{data.answerData.answer}</p>
						</div>
						<div>
							<span class="text-slate-600">Length:</span>
							<p class="text-slate-900 font-mono text-lg mt-1">{data.answerData.answer.length} characters</p>
						</div>
					</div>
					<div class="mt-4 pt-4 border-t border-slate-200">
						<span class="text-slate-600 text-sm">Quick actions:</span>
						<div class="flex flex-wrap gap-2 mt-2">
							<button
								onclick={copyEquation}
								type="button"
								class="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-sm font-medium transition-colors"
							>
								{copied ? 'Copied' : 'Copy Equation'}
							</button>
							<a
								href="https://www.nerdlegame.com/"
								target="_blank"
								rel="noopener noreferrer"
								class="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm font-medium transition-colors"
							>
								Play on Nerdle
							</a>
						</div>
					</div>
				</div>
			{/if}

			<div class="flex flex-wrap justify-between items-center gap-3 text-sm">
				<a href={`/nerdle-archive?date=${data.answerData.date}`} class="text-violet-700 hover:text-violet-600 font-medium transition-colors">
					View in Nerdle Archive
				</a>
				<div class="flex flex-wrap gap-4">
					<a href="/nerdle-archive" class="text-slate-700 hover:text-slate-900 transition-colors">Browse Archive -></a>
				</div>
			</div>
		</div>
	</div>
</main>
