...
if (!indexNowKey) {
  fs.writeFileSync(
    reportPath,
    JSON.stringify({ status: 'skipped', reason: 'missing-indexnow-key', submittedCount: 0 }, null, 2)
  );
  process.exit(0);
}
...
if (!urlList.length) {
  fs.writeFileSync(
    reportPath,
    JSON.stringify({ status: 'skipped', reason: 'no-urls', submittedCount: 0 }, null, 2)
  );
  process.exit(0);
}
...
try {
  ...
  console.log(`IndexNow ${response.ok ? 'submitted' : 'failed'} with status ${response.status}.`);
} catch (error) {
  fs.writeFileSync(
    reportPath,
    JSON.stringify(
  ...
  console.warn(`IndexNow submit failed: ${error instanceof Error ? error.message : String(error)}`);
}
