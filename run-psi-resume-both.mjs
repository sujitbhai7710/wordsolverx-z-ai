...
const existing = new Set(readdirSync(OUTDIR).filter(f => f.endsWith('.json')));
const remaining = [];
for (const url of all) {
  const slug = slugify(url);
  if (!existing.has(`${slug}-mobile.json`)) remaining.push({ url, strategy: 'mobile' });
  if (!existing.has(`${slug}-desktop.json`)) remaining.push({ url, strategy: 'desktop' });
}

console.error(`Resuming: ${remaining.length} tests remaining`);
...
for (let i = 0; i < remaining.length; i += batchSize) {
  ...
  const batchResults = await Promise.all(batch.map(r => runPSI(r.url, r.strategy)));
