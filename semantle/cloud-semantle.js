...
function solveStory(guesses, puzzleNumber, won, hints_used) {
    ...
    if (guess_count === 0) {
        return `I gave up on ${gameName} ${puzzleNumber} without even guessing once. https://legacy.semantle.com/`;
    }
...
let Semantle = (function () {
    ...
    function endGame(won, countStats) {
        ...
        if (won) {
            response = `<p><b>You found it in ${guesses.length}!  The secret word is ${secret}</b>.  Feel free to keep entering words if you are curious about the similarity to other words. You can see the nearest words <a href="/nearby_1k/${secretBase64}">here</a>.</p>${share}`
            window.mplConfetti.start();
        } else {
            response = `<p><b>You gave up!  The secret word is: ${secret}</b>.  Feel free to keep entering words if you are curious about the similarity to other words. You can see the nearest words <a href="/nearby_1k/${secretBase64}">here</a>.</p>${share}`;
        }
